...
print "\n\nSTART downloading - current date: " . `date` ;
...
print "FIELDS main=$mainv daily=$dailyv bytecode=$bytecodev safebrowsing=$safebrowsingv\n";

updateFile('main',$mainv);
updateFile('daily',$dailyv);
updateFile('bytecode',$bytecodev);
updateFile('safebrowsing',$safebrowsingv);
...
sub updateFile {
  ...
  if  ( ! -z "$file.cvd" ) {
    ...
    if ( $old > 0 ) {
            ...
            for (my $count = $old + 1 ; $count <= $currentversion; $count++) {
                    print `wget -nH -nd -N -nv --connect-timeout=10 $mirror/$file-$count.cdiff 2>&1`;
            } 
    } else {
  ...
  print `cd temp && wget -nH -nd -N -nv --connect-timeout=10 $mirror/$file.cvd 2>&1`;
